...
3. `docs/DEVELOPMENT.md` — development execution, verification cadence, handoff, concurrency, workspace/commit discipline, data safety, and publication contract.
...
`docs/DEVELOPMENT.md` owns the development lifecycle and is the only execution contract: fresh `origin/main` → detached isolated worktree → bounded implementation + explicit staging scope → nearest faithful proof → JIT fetch/reconcile → non-destructive publication → remote read-back → published-only workspace cleanup. External coordination ledgers/inboxes are not required by the normal path and never override live Git state.
